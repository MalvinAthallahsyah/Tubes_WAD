        ...
        <div class="md:flex md:space-x-8">
            ...
            <div class="md:w-1/3 mb-6 md:mb-0">
                <div class="aspect-square bg-gray-200 dark:bg-gray-700 rounded-lg flex items-center justify-center overflow-hidden">
                    ...
                    @else
                        {{-- Fallback SVG if no image_path or image file doesn't exist --}}
        ...
        <div class="md:flex md:space-x-8 items-start">
            ...
            <div class="md:w-2/3 space-y-8">
                @forelse($product->reviews as $review)
                    <div class="flex space-x-3 sm:space-x-4 py-6 border-b border-gray-200 dark:border-gray-700 last:border-b-0 last:pb-0">
                        <div class="flex-shrink-0">
                            <span class="inline-flex items-center justify-center h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 font-semibold">
                        ...
                        <div class="flex-grow">
                            <div class="flex items-baseline justify-between">
                                <div>
                                    <span class="font-semibold text-gray-800 dark:text-gray-100 text-sm">{{ $review->user->name ?? 'Telkom Student' }}</span>
                                    <span class="ml-2 inline-flex items-center text-xs text-green-700 dark:text-green-300 bg-green-100 dark:bg-green-600/30 px-2 py-0.5 rounded-full font-medium">
                                        <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd" /></svg>
                            ...
                            <div class="flex items-center my-1.5">
                                @for ($s = 1; $s <= 5; $s++)
                                    <svg class="w-5 h-5 {{ $s <= $review->rating ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-500' }}" fill="currentColor" viewBox="0 0 20 20"><path d="M10 15l-5.878 3.09 1.123-6.545L.489 6.91l6.572-.955L10 0l2.939 5.955 6.572.955-4.756 4.635 1.123 6.545z"/></svg>
                                @endfor
                                <span class="ml-2 text-xs font-semibold text-gray-700 dark:text-gray-300">({{ $review->rating }}/5 Stars)</span>
                            </div>

                            @if(isset($review->title) && $review->title)
                                <h5 class="font-semibold text-md text-gray-800 dark:text-gray-200 mt-2 mb-1.5">{{ $review->title }}</h5>
                            @endif
                            ...
                            <div class="mt-4 flex items-center space-x-3 text-xs">
                                <button class="flex items-center text-gray-500 hover:text-indigo-600 dark:text-gray-400 dark:hover:text-indigo-300 font-medium transition-colors duration-150">
                                    ...
                                    Helpful
                                    <span class="ml-1.5 text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded-sm">(0)</span>
                                </button>
                                <span class="text-gray-300 dark:text-gray-600">·</span>
                                @auth
                                    @if(Auth::id() == $review->user_id || (Auth::user()->isAdmin && Auth::user()->isAdmin()))
                                        <a href="{{ route('reviews.edit', $review) }}" class="font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors duration-150">Edit</a>
                                        <form action="{{ route('reviews.destroy', $review) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this review?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="font-medium text-red-600 hover:text-red-500 dark:text-red-500 dark:hover:text-red-400 transition-colors duration-150">Delete</button>
                                        </form>
                                    @else
                                        <button class="font-medium text-gray-500 hover:text-orange-600 dark:text-gray-400 dark:hover:text-orange-300 transition-colors duration-150">Report</button>
                                    @endif
                                @else
                                     <a href="{{-- route('login') --}}" class="font-medium text-gray-400 dark:text-gray-500 hover:text-indigo-500" title="Login to interact">Report</a>
                                @endauth
                            </div>
                        </div>
                    </div>
