...
@section('content')
    {{-- Container utama, tambahkan padding standar jika layout utama belum punya --}}
    <div class="container mx-auto py-8 px-4">
        {{-- Menghapus dark:bg-gray-800 dari kartu utama --}}
            ...
            <form action="{{ isset($review) ? route('reviews.update', $review) : route('reviews.store') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <div class="mt-1 flex flex-row-reverse justify-end rating-stars-group">
                        ...
                        {{-- Menghapus dark:text-gray-600 dari label bintang --}}
                        <label for="rating{{ $i }}" class="star-label cursor-pointer p-0.5 sm:p-1 text-gray-300">
                ...
                {{-- Review Title (Optional) --}}
                 <div>
                    ...
                    <label for="review_title" class="block text-sm font-medium text-gray-700 mb-1">
                        Feedback Headline <span class="text-xs text-gray-500">(Optional)</span> {{-- Menghapus dark:text-gray-400 --}}
                    </label>
                    {{-- Menghapus dark:border-gray-600, dark:bg-gray-700, dark:text-white dari input --}}
                ...
                <div>
                    ...
                    <label for="comment" class="block text-sm font-medium text-gray-700 mb-1">
                        Detailed Feedback <span class="text-xs text-gray-500">(Optional, but very helpful!)</span> {{-- Menghapus dark:text-gray-400 --}}
                    </label>
                    {{-- Menghapus dark:border-gray-600, dark:bg-gray-700, dark:text-white dari textarea --}}
                    <textarea name="comment" id="comment" rows="5"
                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('comment') border-red-500 @enderror"
                ...
                <div>
                    {{-- Menghapus dark:text-gray-300 dari label --}}
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        Add Photos <span class="text-xs text-gray-500">(Optional, max 3 files)</span> {{-- Menghapus dark:text-gray-400 --}}
                    </label>
                    {{-- Menghapus dark:border-gray-600 dari area upload --}}
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                        <div class="space-y-1 text-center">
                            {{-- Menghapus dark:text-gray-500 dari SVG --}}
                            <svg class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true">
                                <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8m-12 4h.02" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                            {{-- Menghapus dark:text-gray-400 dari teks --}}
                            <div class="flex text-sm text-gray-600">
                                {{-- Menghapus dark:bg-gray-800, dark:text-indigo-400, dark:hover:text-indigo-300, dark:focus-within:ring-offset-gray-800 dari label upload --}}
                                <label for="review_images" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                ...
                {{-- Menghapus dark:border-gray-700, dark:text-gray-400, dark:focus:ring-offset-gray-800 dari tombol --}}
                <div class="pt-6 flex items-center justify-end space-x-4 border-t border-gray-200">
                    <a href="{{ $type === 'product' ? route('products.show', $reviewable->id) : route('sellers.show', $reviewable->id) }}" class="text-sm text-gray-600 hover:underline">Cancel</a>
                    <button type="submit"
                            class="inline-flex items-center justify-center px-6 py-2.5 bg-red-600 hover:bg-red-700 text-white font-semibold text-sm rounded-md shadow-sm transition duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">
